...
import { Settings, Database, Trash2, Plus, TestTube, MapPin } from 'lucide-react';
...
interface DevMenuProps {
  ...
  mapFeatureEnabled: boolean;
  onMapFeatureToggle: (enabled: boolean) => void;
}

export function DevMenu({ onClearDatabase, onPopulateExampleData, mapFeatureEnabled, onMapFeatureToggle }: DevMenuProps) {
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label htmlFor="map-feature" className="text-sm font-medium">
                      Map Feature
                    </Label>
                    <p className="text-xs text-muted-foreground">
                      Enable map view for events with locations
                    </p>
                  </div>
                  <Switch
                    id="map-feature"
                    checked={mapFeatureEnabled}
                    onCheckedChange={onMapFeatureToggle}
                  />
                </div>
              </CardContent>
            </Card>
            ...
            <div className="text-xs text-muted-foreground space-y-1">
              ...
              <p><strong>Map Feature:</strong> Experimental feature to show events on an interactive map.</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
